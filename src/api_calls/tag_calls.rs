use crate::{request::API_URL_BASE, structures::tag_structs::*, Ferinth, Result};

impl Ferinth {
    /// List the categories a project can take
    pub async fn list_categories(&self) -> Result<Vec<Category>> {
        Ok(self
            .get(API_URL_BASE.join("tag/")?.join("category")?)
            .await?
            .json()
            .await?)
    }
    ...
    pub async fn list_loaders(&self) -> Result<Vec<Loader>> {
        Ok(self
            .get(API_URL_BASE.join("tag/")?.join("loader")?)
            .await?
            .json()
            .await?)
    }
    ...
    pub async fn list_game_versions(&self) -> Result<Vec<GameVersion>> {
        Ok(self
            .get(API_URL_BASE.join("tag/")?.join("game_version")?)
            .await?
            .json()
            .await?)
    }
    ...
    pub async fn list_licenses(&self) -> Result<Vec<License>> {
        Ok(self
            .get(API_URL_BASE.join("tag/")?.join("license")?)
            .await?
            .json()
            .await?)
    }
    ...
    pub async fn list_donation_platforms(&self) -> Result<Vec<DonationPlatform>> {
        Ok(self
            .get(API_URL_BASE.join("tag/")?.join("donation_platform")?)
            .await?
            .json()
            .await?)
    }
    ...
    pub async fn list_report_types(&self) -> Result<Vec<String>> {
        Ok(self
            .get(API_URL_BASE.join("tag/")?.join("report_type")?)
            .await?
            .json()
            .await?)
    }
