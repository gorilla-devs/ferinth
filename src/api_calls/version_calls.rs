...
/// # Ok(()) }
/// ```
pub async fn list_versions(client: &Ferinth, project_id: &str) -> Result<Vec<Version>> {
...
/// # Ok(()) }
/// ```
pub async fn get_version(client: &Ferinth, version_id: &str) -> Result<Version> {
    check_id_slug(version_id)?;
    Ok(
        request(client, API_URL_BASE.join("version/")?.join(version_id)?)
            .await?
            .json()
            .await?,
    )
...
/// # Ok(()) }
/// ```
pub async fn get_version_from_file_hash(client: &Ferinth, file_hash: &str) -> Result<Version> {
    check_sha1_hash(file_hash)?;
    Ok(
        request(client, API_URL_BASE.join("version_file/")?.join(file_hash)?)
            .await?
            .json()
            .await?,
    )
...
/// # Ok::<(), ferinth::Error>(()) }
/// ```
pub async fn download_version_file(client: &Ferinth, version_file: &VersionFile) -> Result<Bytes> {
    Ok(request(client, &version_file.url).await?.bytes().await?)
}
